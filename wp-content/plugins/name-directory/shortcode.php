...
function name_directory_show_submit_form($directory, $overview_url)
{
    ...
    if(! empty($name_directory_settings) && ! empty($name_directory_settings['enable_recaptcha']))
    {
        ...
        $recaptcha_html = "<div class='name_directory_forminput'><br><div class='g-recaptcha' data-sitekey='" . $name_directory_settings['recaptcha_sitekey'] . "'></div>";
    }
        ...
        else
        {
            $db_success = $wpdb->insert(
                ...
                array(
                    ...
                    'name'          => stripslashes_deep(wp_strip_all_tags($_POST['name_directory_name'])),
                    'letter'        => name_directory_get_first_char($_POST['name_directory_name']),
                    'description'   => stripslashes_deep(wp_strip_all_tags($_POST['name_directory_description'])),
                    'published'     => 0,
                    'submitted_by'  => stripslashes_deep(wp_strip_all_tags($_POST['name_directory_submitter'])),
                ),
                array('%d', '%s', '%s', '%s', '%d', '%s')
            );
    ...
    $form = <<<HTML
        <form method='post' name='name_directory_submit'>
            ...
            <div class='name_directory_form_result {$result_class}'>{$form_result}</div>
